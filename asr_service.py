...
if os.environ.get("ASR_VAD", "").lower() == "silero":
    try:
        ...
        print("ERROR: Silero VAD requested but not available. Install with: pip install silero-vad")
        sys.exit(1)
else:
    # Default priority: WebRTC -> Silero
    try:
        import webrtcvad
        VAD_ENGINE = "webrtc"
    except ImportError:
        try:
            ...
            print("ERROR: No VAD engine found. Install with: pip install webrtcvad silero-vad")
            sys.exit(1)
...
class AudioBuffer:
    ...
    def detect_speech(self, audio_chunk: np.ndarray) -> bool:
        ...
        try:
            ...
            # Debug output for audio levels and VAD decisions
            if Config.SHOW_VAD_ACTIVITY and audio_level > 0.001:  # Only show if there's some audio
                status = "🟢SPEECH" if is_speech else "🔵silence"
                print(f"{status} | Level: {audio_level:.4f}")
    ...
    def add_audio(self, audio_chunk: np.ndarray, timestamp: float) -> Optional[Tuple[np.ndarray, float, float]]:
        """
...
class ASRService:
        ...
        while self.running:
            try:
                ...
                if result is not None:
                    ...
                    print(f"🎯 Speech detected: {duration:.1f}s segment, transcribing...")
    ...
    def _transcribe_and_log(self, audio_data: np.ndarray, start_time: float, end_time: float):
