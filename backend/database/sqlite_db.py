...
class DatabaseManager:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db:
            try:
                await db.execute("""
                    ...
                    VALUES (?, ?, ?)
                """, (student_data['id'], student_data['name'], student_data['student_id']))
    ...
    async def get_videos() -> List[Dict[str, Any]]:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute("SELECT * FROM videos ORDER BY order_index") as cursor:
    ...
    async def create_session(session_data: Dict[str, Any]) -> str:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db:
            await db.execute("""
                ...
                VALUES (?, ?, ?, ?)
            """, (session_data['id'], session_data['student_id'], 
                  session_data['video_id'], session_data.get('status', 'started')))
            await db.commit()
    ...
    async def save_asr_result(asr_data: Dict[str, Any]) -> int:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db:
            ...
            """, (
                asr_data['session_id'], asr_data['student_id'], asr_data['video_id'],
                asr_data['transcript'], asr_data.get('ground_truth', ''),
                asr_data.get('confidence', 0.0), asr_data.get('wer', 1.0),
                asr_data.get('cer', 1.0), asr_data.get('similarity_score', 0.0),
    ...
    @staticmethod
    async def get_student_asr_results(student_id: str, video_id: str = None) -> List[Dict[str, Any]]:
