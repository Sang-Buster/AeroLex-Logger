...
def test_vad_engines(audio_data):
    ...
    try:
        ...
        if total_frames > 0:
            ...
            print(f"WebRTC VAD: {speech_frames}/{total_frames} frames detected as speech ({speech_percentage:.1f}%)")
        else:
    ...
    try:
        ...
        if speech_timestamps:
            ...
            print(f"Silero VAD: {len(speech_timestamps)} speech segments, {speech_percentage:.1f}% speech")
        else:
...
def main():
    ...
    print("- WebRTC VAD is sensitive to audio quality - try Silero VAD if issues persist")
