...
class StudentService:
    ...
    async def get_student_progress(student_id: str) -> Dict[str, Any]:
        ...
        scores = [r['similarity_score'] for r in asr_results if r['similarity_score'] is not None]
    ...
    @staticmethod
    async def update_video_progress(student_id: str, video_id: str, completed: bool = False, score: float = 0.0):
        ...
        async with await DatabaseManager.get_connection() as db:
            ...
            await db.execute("""
                ...
                SELECT 
                    ...
                    CURRENT_TIMESTAMP
            """, (student_id, video_id, student_id, video_id, completed, 
                  student_id, video_id, score, student_id, video_id))
        ...
        if completed:
            ...
            print(f"🎯 Student {student_id} completed video {video_id} (score: {score:.3f})")
