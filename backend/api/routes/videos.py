    ...
    try:
        async with await DatabaseManager.get_connection() as db:
            ...
            async with db.execute("SELECT * FROM videos WHERE id = ?", (video_id,)) as cursor:
                ...
                video_file_path = VideoService.get_video_file_path(video_data['filename'])
                video_data['file_exists'] = video_file_path is not None
                video_data['video_url'] = f"/videos/{video_data['filename']}" if video_file_path else None
...
async def initialize_video_database():
    ...
    try:
        ...
        return {
            "success": True,
            "message": "Video database initialized successfully"
        }
