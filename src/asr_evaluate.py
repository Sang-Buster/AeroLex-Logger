...
def find_best_match(ground_truth_segments: List[str], transcript: str, 
                   threshold: float = 0.3) -> Tuple[Optional[str], float, int]:
    """
...
def evaluate_single_pair(reference: str, hypothesis: str) -> Dict:
    ...
    similarity = SequenceMatcher(None, normalize_text(reference), 
                                normalize_text(hypothesis)).ratio()
...
def evaluate_asr_results(ground_truth_file: str, asr_results_file: str, 
    ...
    for i, asr_result in enumerate(asr_results):
        ...
        if best_match:
            ...
            print(f"✅ Match {i+1}: WER={metrics['wer']:.3f}, CER={metrics['cer']:.3f}, Sim={similarity:.3f}")
        else:
            print(f"❌ No match {i+1}: '{transcript[:50]}...' (best sim: {similarity:.3f})")
            evaluation.update({
                'reference': '',
                'wer': 1.0,
                'cer': 1.0,
                ...
                'wer_details': {'total_errors': len(transcript.split()), 'total_words': 0}
            })
    ...
    if matched_evaluations:
        ...
        avg_similarity = sum(e['similarity_score'] for e in matched_evaluations) / len(matched_evaluations)
        avg_word_accuracy = sum(e['word_accuracy'] for e in matched_evaluations) / len(matched_evaluations)
        avg_char_accuracy = sum(e['char_accuracy'] for e in matched_evaluations) / len(matched_evaluations)
    else:
    ...
    overall_stats = {
        ...
        'match_rate': len(matched_evaluations) / len(evaluations) if evaluations else 0.0,
        'coverage_rate': len(matched_gt_indices) / len(ground_truth) if ground_truth else 0.0,
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate ASR results using Levenshtein distance and other metrics")
    parser.add_argument("ground_truth", nargs='?', help="Path to ground truth file (txt, json)")
    ...
    parser.add_argument("--compare", nargs=2, metavar=('REF', 'HYP'),
    ...
    if args.compare:
        ...
        if not args.ground_truth or not args.asr_results:
            parser.error("ground_truth and asr_results are required for batch evaluation")
