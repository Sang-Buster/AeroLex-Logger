...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Show planned changes without executing them")
    parser.add_argument("--execute", action="store_true", help="Execute the renaming without prompting")
    ...
    if not args.execute:
        print("-" * 80)
        try:
            response = input(f"❓ Rename {len(rename_plan)} video files? (y/N): ").strip().lower()
            if response != 'y':
